...
def get_parser():
    ...
    parser.add_argument(
        "--train_img_dirs",
        default="/home/ye/CODE/MY/YOLOv5-LPRNet-Licence-Recognition/tmp/datasets/brazil/rec/train",
        help="the train images path",
    )
    parser.add_argument(
        "--test_img_dirs",
        default="/home/ye/CODE/MY/YOLOv5-LPRNet-Licence-Recognition/tmp/datasets/brazil/rec/val",
        help="the test images path",
    )
    parser.add_argument("--dropout_rate", default=0.5, help="dropout rate.")
    parser.add_argument(
        "--learning_rate", default=0.01, help="base value of learning rate."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--phase_train", default=True, type=bool, help="train or test phase flag."
    )
    parser.add_argument(
        "--num_workers",
        default=0,
    ...
    parser.add_argument(
        "--test_interval", default=500, type=int, help="interval for evaluate"
    )
    parser.add_argument("--momentum", default=0.9, type=float, help="momentum")
    parser.add_argument(
        "--weight_decay", default=2e-5, type=float, help="Weight decay for SGD"
    )
    parser.add_argument(
        "--lr_schedule",
    ...
    parser.add_argument(
        "--save_folder",
        default="/home/ye/CODE/MY/YOLOv5-LPRNet-Licence-Recognition/tmp/runs/brazil/",
...
def train():
    ...
    # https://blog.csdn.net/benben044/article/details/125146621
    # 输出序列长度T尽量在模型设计时就要考虑模型需要预测的最长序列，如需要预测的最长序列其长度为l，则理论上T应大于等于2*l+1，这是因为CTCLoss假设在最坏情况下每个真实标签前后都至少有一个空白标签进行隔开以区分重复项。
    T_length = 18  # args.lpr_max_len # 18
    ...
    if args.pretrained_model:
        ...
        logger.info("initial net weights successful!")
    ...
    for iteration in range(start_iter, max_iter):
        ...
        if (iteration + 1) % args.test_interval == 0:
            acc = test_lprnet.Greedy_Decode_Eval(lprnet, test_dataset, args)
            # lprnet.train() # should be switch to train mode
            # save best model
            if acc > best_acc:
                best_acc = acc
                torch.save(lprnet.state_dict(), args.save_folder + "lprnet-best.pth")
                logger.info(
                    "== Save best model ==>  Epoch:{} Acc:{}".format(epoch, acc)
                )
        ...
        lr = adjust_learning_rate(
            optimizer, epoch, args.learning_rate, args.lr_schedule
        )
    ...
    test_lprnet.Greedy_Decode_Eval(lprnet, test_dataset, args)

    # save final parameters
    torch.save(lprnet.state_dict(), args.save_folder + "lprnet-latest.pth")

    logger.info("== Save best model ==>  Epoch:{} Acc:{}".format(epoch, acc))
